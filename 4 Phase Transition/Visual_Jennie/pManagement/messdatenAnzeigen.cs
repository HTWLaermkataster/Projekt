...
namespace pManagement
{
    public partial class messdatenAnzeigen : Form
    {
        ...
        public messdatenAnzeigen(XElement root)
        {
            ...
            {
                Messwert temp = new Messwert(
                                item.Element("Ort").Value, Convert.ToInt16(item.Element("MaxWert").Value),
                                Convert.ToInt16(item.Element("MinWert").Value), Convert.ToInt16(item.Element("Average").Value),
            ...
            textBox5.Text = Convert.ToString(Mlist.First().Average);
            min = Mlist.First().Average;
            max = Mlist.First().Average;
        ...
        private void weiter_button_Click(object sender, EventArgs e)
        {
            ...
            textBox5.Text = Convert.ToString(Marray[i].Average);
